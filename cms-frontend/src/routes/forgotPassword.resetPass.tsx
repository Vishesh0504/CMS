...
async function onSubmit(values: z.infer<typeof formSchema>) {
  ...
  console.log(data);
  try {
    const res = await axios.post(`${URL_ORIGIN}/auth/forgotPassword/resetPass`, data);
    if (res.status === 200) {
  ...
  return(
    <div className="flex justify-center py-48 bg-hero-pattern bg-cover h-screen">
      <div className="border border-neutral-300 rounded-md shadow px-8 py-6 h-fit w-96 flex flex-col gap-4">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="Enter the new password" type="password" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input placeholder="Re-enter the new password" {...field} type="password" />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
        ...
        <div className="flex justify-end gap-2">Try<button className="underline " onClick={()=>navigate({to:"/"})}>Login?</button></div>
